...
import { Svg, Path, G } from 'react-native-svg'; // If you're trying to import Svg-related components
...
const BodySvg = () => {
  ...
  // Function to handle click on a muscle part
  const handleMuscleClick = (muscleId) => {
    // Set the selected muscle ID in state
    setSelectedMuscle(muscleId);
    // Perform any other actions needed upon muscle selection
    // For instance, change the color of the selected muscle
  };
  ...
  return (
    <View> 
        ...
        viewBox="0 0 306.59 401.38"
        preserveAspectRatio="xMidYMid meet"
        >
        <G class = "bodyMapMuslce" id="Shoulders">
            <Path 
                onPress={() => handleMuscleClick('Shoulder')} 
                fill={selectedMuscle === 'Shoulder' ? '#FF5E00' : '#484847'}
                class="cls1" 
                id="Shoulder" 
                d="m50.41,101.92c.56-2.7,1.49-6.6,3.04-11.2,1.77-5.27,2.71-7.95,4.78-10.66,3.37-4.4,7.68-6.33,9.86-7.27,8.03-3.48,15.43-2.09,18.42-1.49,2.3.46,4.16,1.09,5.48,1.59-1.82.58-4.45,1.63-7.17,3.59-3.1,2.23-4.19,4.12-7.17,7.87-6.41,8.06-10.54,13.25-15.93,14.14-1.13.19-2.97.19-5.68,1-2.49.74-4.41,1.73-5.63,2.44Z"
            />
             <Path 
                onPress={() => handleMuscleClick('Shoulder')} 
                fill={selectedMuscle === 'Shoulder' ? '#FF5E00' : '#484847'}
        ...
        <G class = "bodyMapMuslce" id="Traps">
            <Path 
                onPress={() => handleMuscleClick('Traps')} 
                fill={selectedMuscle === 'Traps' ? '#FF5E00' : '#484847'}
                class="cls1" 
                id="Traps" 
                d="m79.83,68.65c2.47-1.65,4.62-2.54,5.62-2.96,3.03-1.28,4.54-1.91,5.66-2.5,0,0,1.06-.55,6.05-3.7h0s1.19-1.14,1.22-1.17c.05.84.12,2.03.17,3.46.22,5.91-.15,6.8-.37,7.24-.93,1.87-2.87,3.37-4.89,3.62-.93.12-1.14-.13-3.66-.77-2.95-.75-5.2-1.33-7.87-1.36-2.24-.03-3.67.35-3.82-.1-.06-.17.04-.53,1.89-1.77Z"
            />
            <Path 
                onPress={() => handleMuscleClick('Traps')} 
                fill={selectedMuscle === 'Traps' ? '#FF5E00' : '#484847'}
                class="cls1" 
                id="Traps" 
                d="m144.78,69.07c-2.47-1.65-4.62-2.54-5.62-2.96-3.03-1.28-4.54-1.91-5.66-2.5,0,0-1.06-.55-6.05-3.7h0s-1.19-1.14-1.22-1.17c-.05.84-.12,2.03-.17,3.46-.22,5.91.15,6.8.37,7.24.93,1.87,2.87,3.37,4.89,3.62.93.12,1.14-.13,3.66-.77,2.95-.75,5.2-1.33,7.87-1.36,2.24-.03,3.67.35,3.82-.1.06-.17-.04-.53-1.89-1.77Z"
            />
        </G>
        <G class = "bodyMapMuslce" id="Chest">
            <Path 
                onPress={() => handleMuscleClick('Chest')} 
                fill={selectedMuscle === 'Chest' ? '#FF5E00' : '#484847'}
                class="cls1" 
                id="Chest" 
                d="m62.31,98.53c2.89-1.47,4.95-3.04,6.27-4.18,2.03-1.75,3.35-3.37,5.98-6.57,2.56-3.13,2.49-3.38,4.38-5.53,2.28-2.59,3.57-4.06,5.58-5.48,3.14-2.21,6.08-2.91,9.76-3.78,3.56-.84,5.91-1.4,9.06-.9,2.01.32,5.03.81,6.97,3.19,1.19,1.45,1.4,3.51,1.79,7.57.25,2.64.42,5.94.2,10.56-.21,4.58-.32,6.87-1,8.46-1.74,4.1-5.2,6.22-7.57,7.67-1.6.98-6.01,3.6-12.25,3.63-1.18,0-8.93-.06-14.89-5.28-2.68-2.35-2.99-3.98-6.27-6.17-2.98-1.99-5.95-2.81-8.02-3.19Z"
            />
            <Path 
                onPress={() => handleMuscleClick('Chest')} 
                fill={selectedMuscle === 'Chest' ? '#FF5E00' : '#484847'}
        ...
        <G class = "bodyMapMuslce" id="Lats">
            <Path 
                onPress={() => handleMuscleClick('Lats')} 
        ...
        <G class = "bodyMapMuslce" id="Abs">
            <Path 
                onPress={() => handleMuscleClick('Abs')} 
                fill={selectedMuscle === 'Abs' ? '#FF5E00' : '#484847'}
                class="cls1" 
                id="Abs" 
        ...
        <G class = "bodyMapMuslce" id="Biceps">
            ...
            <Path 
                ...
                class="cls1" 
                id="Biceps" 
                d="m38.51,123.62c.4-1.44,1.16-3.24,2.69-6.82,1.29-3.02,1.94-4.53,2.94-6.32,1.39-2.5,2.52-4.54,4.78-6.57,2.01-1.81,3.94-2.74,4.66-3.06,3.24-1.46,6.03-1.52,8.27-1.54,3.07-.03,4.64-.05,6.12,1.05,1.75,1.29,2.24,3.27,2.64,4.88,1.29,5.23-.9,9.79-1.62,11.23-1.09,2.18-2.28,3.55-3.34,4.78-2.43,2.83-5.95,7.03-10.41,12.75-.66.77-1.68,1.8-3.11,2.74-1.64,1.08-3.07,1.55-3.36,1.64-1.48.47-4.41,1.41-7.1.07-2.05-1.02-2.99-2.88-3.36-3.61-1.89-3.74-.75-7.81.2-11.2Z"
            />
        </G>
        <G class = "bodyMapMuslce" id="Quads"> 
            <Path 
                onPress={() => handleMuscleClick('Quads')} 
                fill={selectedMuscle === 'Quads' ? '#FF5E00' : '#484847'}
                class="cls1" 
                id="Quads" 
                d="m146.27,173.66c.4,2.32,1,5.76,1.79,9.93,1.78,9.38,3.16,15.28,3.88,18.67,2.82,13.19,3.45,24.5,3.73,30.02.75,14.35-.85,13.17-1.2,29.88,0,0-.06,2.93-2.91,11.88-.36,1.13-1.07,3.27-2.54,3.59-.6.13-.96-.13-4.18-.9-1.12-.27-1.44-.32-1.79-.15-.78.39-.81,1.45-1.2,3.44-.52,2.67-.77,4-1.64,5.08-1.22,1.52-3.06,1.88-3.44,1.94-2.9.5-5.24-1.49-6.12-2.24-1.78-1.51-2.9-3.45-4.33-8.07-3.12-10.03-1.64-13.17-3.44-20.61-.19-.8-.62-2.47-.15-4.48.29-1.21.81-2.17,2.39-4.48,1.48-2.17,2.23-3.26,3.29-4.56,3.31-4.06,4.22-11.84,5.38-22.71,1.02-9.57.86-8.56,1.45-13.88,1.61-14.41,2.39-17.88,4-21.6,2.11-4.87,4.89-8.43,7.02-10.76Z"
            />
            <Path 
                onPress={() => handleMuscleClick('Quads')} 
                fill={selectedMuscle === 'Quads' ? '#FF5E00' : '#484847'}
                class="cls1" 
                id="Quads" 
                d="m78.34,173.24c-.4,2.32-1,5.76-1.79,9.93-1.78,9.38-3.16,15.28-3.88,18.67-2.82,13.19-3.45,24.5-3.73,30.02-.75,14.35.85,13.17,1.2,29.88,0,0,.06,2.93,2.91,11.88.36,1.13,1.07,3.27,2.54,3.59.6.13.96-.13,4.18-.9,1.12-.27,1.44-.32,1.79-.15.78.39.81,1.45,1.2,3.44.52,2.67.77,4,1.64,5.08,1.22,1